...
parser.add_argument("--vllm_model", default=None, help="HF AWQ model id for vLLM reference ceiling (optional).")
...
def measure_vllm_decode(vllm_model, batch_sizes, decode_tokens):
    from vllm import LLM, SamplingParams
    llm = LLM(model=vllm_model, quantization="awq_marlin", dtype="float16", gpu_memory_utilization=0.9)
    params = SamplingParams(temperature=0, max_tokens=decode_tokens)
    rows = []
    for bs in batch_sizes:
        ...
        rows.append({"batch_size": bs, "decode_tok_per_s": tok_per_s})
        print(f"  [vllm] bs={bs:>3} decode={tok_per_s:.1f} tok/s")
    return rows
...
def main():
    ...
    if args.vllm_model:
        ...
        try:
            results["vllm_reference"] = {
                "model": args.vllm_model,
                "by_batch": measure_vllm_decode(args.vllm_model, batch_sweep, args.decode_tokens),
            }
        except Exception as e:
            print(f"  vLLM measurement failed: {e}")
